exports.delete = function (req, callback) {
  ...
  exports.downloadFile = function (req, callback) {
    ...
    userFile.findOne({fileId: id, userId: req.user._id}, function (err, data) {
      ...
      if (data && (data.r || data.rw)) {
        File.findById(id, function (err, file) {
          if (file.r.indexOf(req.user.userName) > -1 || file.rw.indexOf(req.user.userName) > -1) {
            callback(err, file);
          }
          else {
            callback("You do not have permission to view this file", null);
          }
        });
      }
      else {
        return callback(new Error("You do not have permission to view this file"));
      }
    });
